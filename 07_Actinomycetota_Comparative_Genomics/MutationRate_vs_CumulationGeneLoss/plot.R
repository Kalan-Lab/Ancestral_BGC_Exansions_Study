...
ggplot(dat, aes(x=PhyloDist, y=GeneLoss)) + geom_point(aes(color=Family, shape=Clade), size=3, show.legend=F) + geom_smooth(method='lm', show.legend=F) + theme_bw() + scale_color_manual(values=colors) + theme(legend.position='bottom') + xlab("") + ylab("") + theme(text = element_text(size=20))
dev.off()
