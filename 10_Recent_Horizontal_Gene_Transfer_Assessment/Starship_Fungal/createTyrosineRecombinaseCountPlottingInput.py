...
genome_tyr_prots = defaultdict(set)
with open('HMMSearch_Results.txt') as ohrf:
    for line in ohrf:
        line = line.strip()
        ...
        if dom == 'DUF3435':
            g = ls[0].split('|')[0]
            genome_tyr_prots[g].add(ls[0])

print('\t'.join(['genome', 'clade', 'large_clade', 'ploidy_status', 'morphology', 'DUF3435_counts']))
with open(antismash_file) as oaf:
    for i, line in enumerate(oaf):
        if i == 0: continue
        line = line.strip()
        ls = line.split('\t')
        if ls[0] not in reps: continue
        ...
        ps = ploidy_mapping[c]
        mo = cm_mapping[c]
        print('\t'.join([ls[0], c, lc, ps, mo, str(len(genome_tyr_prots[ls[0]]))]))
