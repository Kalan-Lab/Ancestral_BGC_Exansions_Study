...
def parseAntiSMASHGBKForFunctionAndCompleteness(bgc_gbk):
    ...
    return([like_or_con, metallophore_related, nrps_related, pks_related, bgc_length, complete_status])
...
gca_complete_nrp_or_pks_count = defaultdict(int)
...
for i, s in enumerate(os.listdir(asresdir)):
    ...
    for f in os.listdir(samp_dir):
        ...
        if not '.region' in f: 
            ...
            bgc_path = samp_dir + f
            like, mr, nr, pr, bl, cs = parseAntiSMASHGBKForFunctionAndCompleteness(bgc_path)
            if like: continue
            gca_bgc_count[gca] += 1
            if cs:
            ...
            if nr or pr:
                ...
                if cs:
                    gca_complete_nrp_or_pks_count[gca] += 1

print('Assembly_ID\tPhylum\tGenus\tBGC_Count\tComplete_BGC_Count\tGenome_Size\tBGC_Genome_Prop\tMetallophore_Genome_Prop\tNRPS_Genome_Prop\tPKS_Genome_Prop\tNRPS_or_PKS_Genome_Prop\tNRPS_or_PKS_Prop\tNRPS_or_PKS\tComplete_NRPS_or_PKS')
for gca in gca_total_sum:
    ...
    nrp_or_pks_genome_prop = float(gca_nrp_or_pks_sum[gca])/float(gca_total_sum[gca])
    nrp_or_pks_prop = 'NA'
    nrp_or_pks = '0'
    complete_nrp_or_pks = '0'
    complete_bgc_count = str(gca_complete_bgc_count[gca])
    if int(bgc_count) > 0:
        nrp_or_pks_prop = float(gca_nrp_or_pks_count[gca])/float(bgc_count)
        nrp_or_pks = gca_nrp_or_pks_count[gca]
        complete_nrp_or_pks = gca_complete_nrp_or_pks_count[gca]
    print('\t'.join([str(x) for x in [gca, gca_to_genus[gca], bgc_count, complete_bgc_count, gca_total_sum[gca], bgc_genome_prop, met_genome_prop, nrp_genome_prop, pks_genome_prop, nrp_or_pks_genome_prop, nrp_or_pks_prop, nrp_or_pks, complete_nrp_or_pks]]))
