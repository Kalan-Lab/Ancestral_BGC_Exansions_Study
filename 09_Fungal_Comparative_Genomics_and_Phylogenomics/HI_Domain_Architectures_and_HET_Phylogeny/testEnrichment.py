...
bgc_rich = set([])
all_fungi = set([x.strip() for x in open('HET_Domain_Phylo/Clade_Info/All_Fungi.txt').readlines()])
for cf in ['HET_Domain_Phylo/Clade_Info/Agaricomycetes.txt', 'HET_Domain_Phylo/Clade_Info/BGC_Enriched_Pezizomycotina.txt']:
    with open(cf) as ocf:
        for line in ocf:
            line = line.strip()
            bgc_rich.add(line)
...
sys.stderr.write(str(pval) + '\t' + str(stat) + '\n')
