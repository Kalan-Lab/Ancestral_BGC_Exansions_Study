...
print('\t'.join(['name', 'gca', 'clade', 'bgcome_size', 'genome_size', 'cazy_count', 'protein_count']))
with open(antismash_stats_file) as oasf:
    for line in oasf:
        ...
        bs = ls[4]
        gs = ls[5]
        print('\t'.join([name, gca, name_to_clade[name], bs, gs, str(cazy[gca]), str(len(proteins[gca]))]))
